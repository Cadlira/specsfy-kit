...
function properties(xml) {
    ...
    for (const match of block.matchAll(/<([\w.-]+)>\s*([^<]+?)\s*<\/\1>/g))
        map.set(match[1], match[2].trim());
    return map;
}
function resolveVersion(value, props) {
    if (!value)
        return "unknown";
    const key = value.match(/^\$\{([^}]+)}$/)?.[1];
    return key ? props.get(key) ?? "unknown" : value;
}
function dependencyVersion(xml, artifact, props) {
...
export async function detectJava(files, root) {
    ...
    for (const path of pomFiles) {
        ...
        if (/<modules>/i.test(xml))
            push(result, "maven-multi-module", "present", source, source);
        ...
        if (spring !== "unknown" || /org\.springframework|<artifactId>\s*spring-(?:core|context|mvc|webmvc)/i.test(xml))
            push(result, "spring-framework", spring, source, source, spring === "unknown" ? "medium" : "high");
        const hibernate = dependencyVersion(xml, "hibernate-core", props) ?? resolveVersion(tag(xml, "hibernate.version"), props);
        if (hibernate !== "unknown" || /<artifactId>\s*hibernate-core\s*<\/artifactId>/i.test(xml))
            push(result, "hibernate", hibernate, source, source, hibernate === "unknown" ? "medium" : "high");
        for (const [technology, artifact] of [["junit", "junit"], ["mockito", "mockito-core"], ["assertj", "assertj-core"]]) {
            const version = dependencyVersion(xml, artifact, props);
            if (version)
                push(result, technology, version, source, source);
        }
    }
    for (const path of files.filter((file) => /build\.gradle(?:\.kts)?$/.test(file))) {
        ...
        push(result, "java", java, source, source, java === "unknown" ? "medium" : "high");
        push(result, "gradle", "present", source, source);
        const boot = body.match(/org\.springframework\.boot['\"]?\s*version\s*['\"]([^'\"]+)/)?.[1];
        if (boot)
            push(result, "spring-boot", boot, source, source);
        const spring = body.match(/org\.springframework:spring-(?:core|context|webmvc):([^'\"\s)]+)/)?.[1];
        if (spring)
            push(result, "spring-framework", spring, source, source);
        const hibernate = body.match(/org\.hibernate(?:\.orm)?:hibernate-core:([^'\"\s)]+)/)?.[1];
        if (hibernate)
            push(result, "hibernate", hibernate, source, source);
    }
    for (const path of files.filter((file) => /settings\.gradle(?:\.kts)?$/.test(file))) {
        const body = await readFile(path, "utf8");
        const source = relative(root, path).replaceAll("\\", "/");
        if (/\binclude\s*(?:\(|\s)/.test(body))
            push(result, "gradle-multi-module", "present", source, source);
    }
    return result;
}
